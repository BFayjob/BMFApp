...
export const RefundHistory = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-8 bg-gray-100">
      <h2 className="text-2xl font-bold text-center text-green-900 mb-6">Refund History</h2>
        ...
        <ul className="list-disc ml-4">
          {refundRecords.map((record) => (
            <li key={record.id} className="mb-4">
              ...
              <p>
                Customer: {record.customerName} - Date: {new Date(record.date).toLocaleDateString()}
              </p>
              <ul className="list-disc ml-4">
              ...
              <p className="text-gray-700">Total Refund Amount: ₦{record.totalAmount}</p>
            </li>
          ))}
        </ul>
      ) : (
